// 基础工具实现：对齐分配、nmod/素数、mpn 小函数（含 flint_mpn_cmp_ui_2exp，
// 原 fsd_helpers.c 已并入本文件）、mulmod 精度边界
...
/* cmp(a, b*2^e)：先去掉 a 高位的零，再与移位后的 b*2^e 逐段比较；
   a 不必规范化。仅用于乘法入口处校验 CRT 素数乘积是否足够大 */
int flint_mpn_cmp_ui_2exp(const ulong* a, ulong an, ulong b, ulong e)
{
    ...
    /* b*2^e = (b0 + b1*2^64)*2^(64*q)，其中 b0:b1 是 b 左移 r 位的结果 */
    if (r == 0)
    {
        b0 = b;
        b1 = 0;
