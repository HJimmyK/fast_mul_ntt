// 截断逆向 IFFT（sd_ifft_trunc），sd_fft_trunc 的逆，输出 = 2^L * 原序列
//
// 移植自 FLINT src/fft_small/sd_ifft.c
// (Copyright (C) 2022 Daniel Schultz, LGPL-3.0-or-later)。
// 截断逆变换的公式推导见 doc/fft.jl 的 itfft!（David Harvey 截断 FFT 的逆）。
...
static void CAT4(radix_2_moth_inv_trunc_block, nn, zz, ff)( \
    const sd_fft_ctx_t FSD_UNUSED(Q), \
    ulong FSD_UNUSED(j), \
    double* FSD_UNUSED(X0), double* FSD_UNUSED(X1)) \
{ \
    fsd_abort("radix_2_moth_inv_trunc_block: 不支持该截断组合 n=" #nn ", z=" #zz ", f=" #ff); \
}
...
static void radix_2_moth_inv_trunc_block_1_1_0(
    const sd_fft_ctx_t Q,
    ulong FSD_UNUSED(j),
    double* X0, double* FSD_UNUSED(X1))
{
    VECND n    = VECNOP(set_d)(Q->p);
    VECND ninv = VECNOP(set_d)(Q->pinv);
    ulong i = 0; do {
...
{ \
   T Z0 = x0, Z1 = x1; \
   x0 = T##_##reduce_to_pm1n(T##_##add(Z0, Z1), n, ninv); \
   x1 = T##_##mulmod(T##_##sub(Z1, Z0), w0, n, ninv); \
}
...
{ \
    ...
    Y2 = T##_##sub(X0, X1); \
    Y3 = T##_##sub(X3, X2); \
    Y2 = T##_##mulmod(Y2, ww0, n, ninv); \
    Y3 = T##_##mulmod(Y3, ww1, n, ninv); \
    Z0 = T##_##add(Y0, Y1); \
    Z1 = T##_##sub(Y3, Y2); \
    Z2 = T##_##sub(Y1, Y0); \
    Z3 = T##_##add(Y3, Y2); \
    x0 = T##_##reduce_to_pm1n(Z0, n, ninv); \
    x1 = Z1; \
    x2 = T##_##mulmod(Z2, w0, n, ninv); \
    x3 = T##_##mulmod(Z3, w0, n, ninv); \
}
...
{ \
    ...
    Y2 = T##_##sub(X0, X1); \
    Y3 = T##_##sub(X2, X3); \
    Y0 = T##_##reduce_to_pm1n(Y0, n, ninv); \
    Y1 = T##_##reduce_to_pm1n(Y1, n, ninv); \
    Y2 = T##_##reduce_to_pm1n(Y2, n, ninv); \
    Y3 = T##_##mulmod(Y3, ww1, n, ninv); \
    x0 = T##_##add(Y0, Y1); \
    x2 = T##_##sub(Y0, Y1); \
    x1 = T##_##sub(Y2, Y3); \
    ...
    double* X0, double* X1, double* X2, double* X3)
{
    ...
    double rw = (j == 0) ? Q->w2tab[0][1] : Q->w2tab[1+j_bits][2*j_r+1];
...
static void radix_4_moth_inv_trunc_block_3_3_0(
    const sd_fft_ctx_t Q,
    ulong j, ulong j_bits,
    double* X0, double* X1, double* X2, double* FSD_UNUSED(X3))
{
    ulong j_mr = n_pow2(j_bits) - 1 - j;
    VECND n    = VECNOP(set_d)(Q->p);
    VECND ninv = VECNOP(set_d)(Q->pinv);
    ...
    double* X0, double* X1, double* X2, double* X3)
{
    ...
    VECND n    = VECNOP(set_d)(Q->p);
    VECND ninv = VECNOP(set_d)(Q->pinv);
    double wi = (j == 0) ? -1.0 : Q->w2tab[1+j_bits][2*j_mr+1];
...
static void radix_4_moth_inv_trunc_block_2_2_0(
    const sd_fft_ctx_t Q,
    ulong j, ulong j_bits,
    double* X0, double* X1, double* FSD_UNUSED(X2), double* FSD_UNUSED(X3))
{
    ulong j_mr = n_pow2(j_bits) - 1 - j;
    VECND n    = VECNOP(set_d)(Q->p);
    VECND ninv = VECNOP(set_d)(Q->pinv);
    ...
    double* X0, double* X1, double* X2, double* X3)
{
    ...
    double W  = (j == 0) ? 1.0 : Q->w2tab[1+j_bits][2*j_r];
    VECMD n    = VECMOP(set_d)(Q->p);
    VECMD ninv = VECMOP(set_d)(Q->pinv);
    VECMD f2 = VECMOP(set_d)(2);
    ...
    double* X0, double* X1, double* X2, double* X3)
{
    ...
    VECND w  = VECNOP(set_d)((j == 0) ? 1.0 : Q->w2tab[1+j_bits][2*j_r]);
    ulong i = 0; do {
        VECND a, b, c, d;
        a = VECNOP(load)(X0+i);
        b = VECNOP(load)(X1+i);
...
static void radix_4_moth_inv_trunc_block_1_1_1(
    const sd_fft_ctx_t Q,
    ulong FSD_UNUSED(j), ulong FSD_UNUSED(j_bits),
    double* X0, double* X1, double* FSD_UNUSED(X2), double* FSD_UNUSED(X3))
{
    VECND n    = VECNOP(set_d)(Q->p);
    VECND ninv = VECNOP(set_d)(Q->pinv);
...
static void radix_4_moth_inv_trunc_block_1_1_0(
    const sd_fft_ctx_t Q,
    ulong FSD_UNUSED(j), ulong FSD_UNUSED(j_bits),
    double* X0, double* FSD_UNUSED(X1), double* FSD_UNUSED(X2), double* FSD_UNUSED(X3))
{
    VECND n    = VECNOP(set_d)(Q->p);
    VECND ninv = VECNOP(set_d)(Q->pinv);
    VECND f = VECNOP(set_d)(4.0);
    ...
    int f)
{
    FSD_ASSERT(f == 0 || f == 1);
    FSD_ASSERT(n <= z);
    FSD_ASSERT(1 <= z && z <= n_pow2(k));
    FSD_ASSERT(1 <= n+f && n+f <= n_pow2(k));
    ...
    int f)
{
    FSD_ASSERT(f == 0 || f == 1);
    FSD_ASSERT(n <= z);
    FSD_ASSERT(1 <= BLK_SZ*z && BLK_SZ*z <= n_pow2(k+LG_BLK_SZ));
    FSD_ASSERT(1 <= BLK_SZ*n+f && BLK_SZ*n+f <= n_pow2(k+LG_BLK_SZ));
